...
int Yolo11Detector::init(const std::string& param_path, const std::string& bin_path, int target_sz)
{
	this->target_size = target_sz;
	
	yolo11.opt.use_vulkan_compute = false;
	yolo11.opt.num_threads = 3;
    // yolo11.opt.use_bf16_storage = true;
	
	if (yolo11.load_param(param_path.c_str()) != 0)
    {
        fprintf(stderr, "Load .param failed: %s\n", param_path.c_str());
        return -1;
    }
    if (yolo11.load_model(bin_path.c_str()) != 0)
    {
        fprintf(stderr, "Load .bin failed: %s\n", bin_path.c_str());
        return -1;
    }
...
int Yolo11Detector::detect(const cv::Mat& bgr, std::vector<Object>& objects)
{
	if (!is_initialized)
    {
        fprintf(stderr, "YOLO11 model load fail!\n");
        return -1;
    }
